...
function dealWith(inPath, up) {
  ...
  return path.join.apply(
    path,
    path.normalize(inPath).split(path.sep).slice(up)
  );
}
...
function copyFiles(args, config, callback) {
  ...
  toStream(
    input.map(function (srcP) {
      return srcP.startsWith("~") ? untildify(srcP) : srcP;
    })
  )
    .pipe(
      through(function (pathName, _, next) {
        var self = this;
    ...
    .pipe(
      through(function (pathName, _, next) {
        fs.stat(pathName, function (err, pathStat) {
          ...
          function done() {
            fs.promises
              .mkdir(path.dirname(outName), { recursive: true })
              .then(() => {
                next(null, {
                  pathName: pathName,
                  pathStat: pathStat,
